...
void calcMinLimit() {
	...
	while (!pq.empty()) {
		...
		if (minLimit[now.y][now.x] < now.limit) continue;
		...
		for (int i = 0; i < 2; i++) {
			...
			if (MAP[now.y][nx] == -1 || MAP[now.y][nx] == 0) continue;
            // 이미 찾은 최소 limit보다 크거나 같으면 skip
			if (minLimit[now.y][nx] <= now.limit) continue;
		...
		for (int i = 0; i < 2; i++) {
            ...
            // 이미 찾은 최소 limit보다 크거나 같으면 skip
			if (minLimit[ny][now.x] <= limit) continue;
...
int main(void) {
	...
	for (int y = 1; y <= N; y++) {
		for (int x = 1; x <= M; x++) {
			...
			if (MAP[y][x] == 2) st = { y, x };  // 출발 지점
			else if (MAP[y][x] == 3) de = { y, x };  // 도착 지점
		}
	}
