...
coord taxi(0,0); // taxi의 위치를 표현

// 현재 위치에서 가장 가까운 승객을 찾아서 승객 index 반환
// 지금 갖고 있는 기름으로 갈 수 없다면 -1 반환
// 갈 수 있는 승객이 없는 경우에도 -1 반환
...
// 승객을 태우고 승객이 원하는 지점으로 갈 수 있는지 확인
// 갈 수 있다면 그에 따른 결과(기름 추가 + taxi 이동)를
// 갈 수 없다(기름이 부족하거나 길이 막혔거나)면 -1 리턴
