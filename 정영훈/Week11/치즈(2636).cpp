...
void Bfs(vector<vector<int>>& Map, pair<int,int> start) {
	...
	while (!q.empty()) {
		...
		for (int d{}; d < 4; ++d) {
			...
			if (nx < 1 || ny < 1 || nx > R || ny > C) continue; // hihi can't go
			if (Map[nx][ny] == 0) {
...
int main() {
	...
	for(;;++time) {
		...
		// cout << time << '\n';
		// PrintMap(Map);
		if (!countOne) break; // all melt
		countOnes.push_back(countOne);
		while (!newBorders.empty()) { // extend border
			pair<int, int> now = newBorders.front(); newBorders.pop();
			Bfs(Map, now);
		}
	}
