...
struct UnionFind {
	...
	bool Union(int a, int b) {
		a = Find(a), b = Find(b);
		if (a ^ b) { // a!=b
			if (par[a] > par[b]) swap(a, b); // a가 b보다 작으면
			par[a] += par[b]; // a 밑에 b
			par[b] = a;
			return true;
		}
...
int main() {
	...
	for (int i{}; i < N; ++i) {
		for (int j{}; j < N; ++j) {
			for (int d{}; d < 4; ++d) {
				...
				if (Map[i][j] == Map[nx][ny]) UF.Union(i * N + j, nx * N + ny);
			}
		}
	}
	...
	// dist, 
	for (int x = 1; x < N; ++x) PQ.push({ dist[x][0] = UF.Size(x * N + 0), x, 0 });
	for (int y = 1; y < N - 1; ++y) PQ.push({ dist[N - 1][y] = UF.Size((N - 1) * N + y), N - 1, y });
