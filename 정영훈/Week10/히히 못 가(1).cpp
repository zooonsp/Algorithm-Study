// BFS + Dijkstra (without make graph info)
#include <iostream>
#include <vector>
#include <string>
#include <queue>
...
int main() {
	...
	while (!PQ.empty()) {
		...
		if (dist[now.c.x][now.c.y] != now.weight) continue;

		for (int dir{}; dir < 8; ++dir) {
			int nx = now.c.x + dx[dir];
			int ny = now.c.y + dy[dir];
			if (RandJ(nx, ny)) continue;
			if (!inside2(nx, ny)) continue;
			int nw{};
			if (Map[nx][ny] == -1) nw = 0;
			else nw = Map[now.c.x][now.c.y] == Map[nx][ny] ? 0 : spaces[Map[nx][ny]];
