...
int nutrient[10][10]{}; // 가장 처음에 양분은 모든 칸에 5만큼 들어있다.
int A[10][10]{}; // 겨울에 추가될 양분
int Ayear[10][10]{}; // 죽은 나무
	...
	for (int k{}; k < K; ++k) { // year
		...
		for (int i{}; i < N; ++i) {
			for (int j{}; j < N; ++j) {
				...
				for(int& a : Map[i][j]) {
					if (enoughNutrient && nowNutrient < a) enoughNutrient = false;
					if (enoughNutrient) {
